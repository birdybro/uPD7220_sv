...
@pytest.mark.rtl
def test_fifo(gdc_seed: SeedContext) -> None:
    build_directory = ROOT / "build" / "sim" / f"fifo-seed-{gdc_seed.seed}"
    ...
    try:
        runner.test(
            hdl_toplevel="upd7220_fifo",
            test_module="tests.cocotb.test_fifo",
            build_dir=build_directory,
            waves=True,
